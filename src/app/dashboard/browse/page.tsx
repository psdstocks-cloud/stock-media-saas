...
export default function BrowsePage() {
  ...
  const getCostColor = (cost: number) => {
    if (cost < 1) return 'bg-green-100 text-green-800'
    if (cost < 3) return 'bg-yellow-100 text-yellow-800'
    ...
    <div className="space-y-6">
          ...
          <Card key={site.id} className="hover:shadow-md transition-shadow">
            <CardHeader className="pb-3">
              <div className="flex items-start justify-between">
                <div>
                  <CardTitle className="text-xl">{site.displayName}</CardTitle>
                ...
                <div className="flex flex-col items-end space-y-2">
                  ...
                  <div className="flex items-center space-x-1 text-sm">
                    ...
                    <span className="font-semibold">{site.cost}</span>
                  </div>
                </div>
              </div>
            </CardHeader>
      ...
      {filteredSites.length === 0 && (
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-semibold text-gray-700 mb-2">No sites found</h3>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {filteredSites.slice(0, 4).map((site) => (
              <div key={site.id} className="text-center p-4 border border-gray-200 rounded-lg hover:border-orange-300 transition-colors cursor-pointer">
                <div className="font-medium text-gray-900">{site.displayName}</div>
                <div className="text-xs text-gray-500 mt-1">{site.cost} points</div>
