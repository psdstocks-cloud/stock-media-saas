...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Direct login attempt:', { email, clientIP })
    ...
    if (!rateLimitResult.success) {
      console.warn('Rate limit exceeded for direct login:', {
        clientIdentifier,
        clientIP,
    ...
    if (!user) {
      console.warn('Direct login attempt with non-existent email:', {
    ...
    if (user.lockedUntil && user.lockedUntil > new Date()) {
      console.warn('Direct login attempt on locked account:', {
        userId: user.id,
        email: user.email,
    ...
    if (!user.password) {
      console.warn('Direct login attempt on OAuth-only account:', {
        userId: user.id,
        email: user.email,
        clientIP,
        timestamp: new Date().toISOString()
    ...
    if (!isValidPassword) {
      ...
      console.warn('Failed direct login attempt:', {
        userId: user.id,
        email: user.email,
    ...
    console.log('Successful direct login:', {
      userId: user.id,
      email: user.email,
      role: user.role,
      clientIP,
